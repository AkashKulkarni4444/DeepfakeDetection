...
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#     # device="cpu"
#     data_tensor = torch.tensor(data).permute(0, 3, 1, 2).float().to(device)

#     for modelName in modelsList:
#         fakeCount = 0
#         rawCount = 0
#         if args.model == 'resnet':
#             model_path = "models_224_"+args.model+"/" + modelName+ '/epoch' + str(args.epoch) + '.pth'  # Assuming PyTorch models have .pth extension
#         elif  args.model == 'mobilenet':
#             model_path = "models_224_"+args.model+"/" + modelName+ '/epoch' + str(args.epoch) + '.pth'  # Assuming PyTorch models have .pth extension
#         model = torch.load(model_path, map_location=device)  # Load model to the same device
#         model.eval()
#         result = model(data_tensor)
#         for i in result:
#             prediction = np.argmax(i.detach().cpu().numpy())  # Move tensor to CPU for numpy operations
#             if prediction == 1:
#                 fakeCount += 1
#             if prediction == 0:
#                 rawCount += 1
